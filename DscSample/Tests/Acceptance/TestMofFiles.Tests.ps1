...
Describe 'Pull Server Deployment' -Tag Acceptance {
    ...
    foreach ($node in $environmentNodes) {
        It "MOF file for node $($node.NodeName) was deployed to $($env:DscConfiguration)" {
            ...
            Get-ChildItem -Path $env:DscConfiguration -Filter "$($node.NodeName).mof" | Where-Object LastWriteTime -gt (Get-Date).AddHours(-1) | Should -Not -BeNullOrEmpty
